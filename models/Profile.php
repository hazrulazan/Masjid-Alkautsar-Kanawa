...
class Profile {
    ...
    public function getProfile() {
        ...
        if (!$profile) {
            $this->pdo->exec("INSERT INTO profiles (name, history, vision, mission) VALUES ('Masjid Al-Ikhlas', '-', '-', '-')");
            $stmt = $this->pdo->query("SELECT * FROM profiles LIMIT 1");
            $profile = $stmt->fetch(PDO::FETCH_ASSOC);
        }
    ...
    public function update($data) {
        $query = "UPDATE profiles SET name = :name, history = :history, vision = :vision, mission = :mission";
        $params = [
            'name' => $data['name'],
